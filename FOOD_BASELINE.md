...
**Phase 2 now registers history shadow mode and a separate exact-reference reuse switch:** see `FOOD_HISTORY.md`. Other future features remain off even if configured on; implementations must register supported modes before activation. No imports are implemented. Live import also requires live grounding. These environment-backed controls require environment refresh/redeployment; they are not a remotely editable flag service. Configuration is captured per execution; persisted cross-deployment job snapshots belong with later durable execution work.
