...
Future switches use `FOOD_HISTORY_SEARCH`, `FOOD_AGENT_TEXT`, `FOOD_AGENT_IMAGE`, `FOOD_GROUNDED_SEARCH`, `FOOD_GROUNDED_IMPORT`, with values `off`, `shadow` or `on` and corresponding `_PERCENT` values from 0–100. Cohorts are stable per user and feature. Invalid settings fail closed. `FOOD_KILL_SWITCH=true` disables rollout features independently of telemetry.

**Phase 2 now registers history shadow mode and a separate exact-reference reuse switch:** see `FOOD_HISTORY.md`. **Phase 3 registers text-agent shadow mode and separate `FOOD_FAST_SELECTOR` / `FOOD_AGENT_FALLBACK` shadow cohorts:** see `FOOD_AGENT.md`. The fallback requires admission to the fast-selector cohort; the new route takes precedence over the older `FOOD_AGENT_TEXT` comparison. Reports now include route and fallback-reason counts. Live text-agent mode and other future features remain off even if configured on; implementations must register supported modes before activation. No agent imports are implemented. Live import also requires live grounding. These environment-backed controls require environment refresh/redeployment; they are not a remotely editable flag service. Configuration is captured per execution; persisted cross-deployment job snapshots belong with later durable execution work.
