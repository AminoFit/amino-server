...
export const processFoodItemQueue = Queue(
  ...
  processFoodItem // 👈 the function that runs when it's called
)

async function processFoodItem(loggedFoodIdString: string) {
  ...
  if (loggedFoodItem.messageId) {
    await ProcessLogFoodItem(loggedFoodItem, openAiData as FoodItemToLog, loggedFoodItem.messageId, loggedFoodItem.User)
  } else {
