...
export default function FoodStats({ foods, user }: { foods: LoggedFoodItemWithFoodItem[]; user: Tables<"User"> }) {
  // set up modal state
  ...
  const cardClasses = "row-span-1 overflow-hidden rounded-lg bg-white p-3 shadow"
  return (
    <div>
      <dl className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-6 mt-5">
        <div className={cardClasses}>
          ...
          <div className="text-sm text-gray-500">
            {totalCalories.toLocaleString("en-us")}/{goalCalories.toLocaleString("en-us")}
          </div>
          <GraphSemiCircle percentage={(totalCalories / goalCalories) * 100} color="#ef470c" label={"Calories"} />
        </div>
        <div className={cardClasses}>
          <div className="flex justify-between">
          ...
          <div className="text-sm text-gray-500">
            {totalFats.toLocaleString("en-us")}/{goalFats.toLocaleString("en-us")}
          </div>
          <GraphSemiCircle percentage={(totalFats / goalFats) * 100} color="#92ba3a" label={"Fats"} />
        </div>
        <div className={cardClasses}>
          <div className="flex justify-between">
          ...
          <div className="text-sm text-gray-500">
            {totalCarbs.toLocaleString("en-us")}/{goalCarbs.toLocaleString("en-us")}
          </div>
          <GraphSemiCircle percentage={(totalCarbs / goalCarbs) * 100} color="#384147" label={"Carbs"} />
        </div>
        <div className={cardClasses}>
          <div className="flex justify-between">
          ...
          <div className="text-sm text-gray-500">
            {totalProtein.toLocaleString("en-us")}/{goalProtein.toLocaleString("en-us")}
          </div>
          <GraphSemiCircle percentage={(totalProtein / goalProtein) * 100} color="#899384" label={"Protein"} />
        </div>
        ...
        <div className={"hidden lg:block col-span-2 md:col-span-4 overflow-hidden rounded-lg bg-white"}>
          ...
          <div className="text-2xl font-bold leading-9 tracking-tight text-gray-900">{user.fitnessGoal}</div>
        </div>
      </dl>
      <GoalsDialog
