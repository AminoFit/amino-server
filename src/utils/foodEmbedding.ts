...
export async function getFoodEmbedding(foodItem: Tables<"FoodItem">): Promise<number[]> {
  ...
  if (foodItem.brand && !textToEmbed.toLowerCase().includes(foodItem.brand.toLowerCase())) {
...
export async function foodToLogEmbedding(foodToLog: FoodItemToLog): Promise<FoodEmbeddingCache> {
  ...
  // Append the brand name with a hyphen if it doesn't already appear in the name
  if (foodToLog.branded && foodToLog.brand && !textToEmbed.toLowerCase().includes(foodToLog.brand.toLowerCase())) {
