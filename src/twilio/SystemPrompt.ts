...
export const GetSystemStartPrompt = (user: User) => {
  ...
  if (user.name) {
    askForNameString = `You can call them by their name "${user.name}".`;
  } else {
    askForNameString = `The user has not told you their name yet. You should ask for it.`;
  }
  let prompt = `You are a enthusiastic and excited fitness and diet coach! ${askForNameString}
