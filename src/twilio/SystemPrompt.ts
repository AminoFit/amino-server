...
export const GetSystemStartPrompt = (user: Tables<"User">) => {
  let nameInfoString = user.fullName ? `You can call the user by their name: "${user.fullName}".` : ``
  ...
  let fitnessGoalString = user.fitnessGoal ? `They are on a journey to ${user.fitnessGoal}.` : ""
  ...
  let calorieGoalString = user.calorieGoal ? `They aim to consume ${user.calorieGoal} calories per day.` : ""

  let unitPreferenceString =
    user.unitPreference === "IMPERIAL" ? `They prefer imperial units.` : `They prefer metric units.`
...
export const GetSystemQuickLogPrompt = (user: Tables<"User">) => {
 ...
 The user wants to log a food item now. They enter the food item.`
  ...
  prompt += `\nThe current time for the user is now ${moment().tz(user.tzIdentifier).format()}`
