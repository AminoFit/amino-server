...
export const mealProposal = z.object({schemaVersion:z.literal(1),
  ...
  components:z.array(z.object({sourceText:z.string().trim().min(1).max(300),
    itemIndexes:z.array(z.number().int().nonnegative()).max(30),
    historySelectionIndexes:z.array(z.number().int().nonnegative()).max(10),
    omitted:z.boolean()}).strict()).max(40),
