...
function updateFoodItemWithMissingInfo(
  ...
  missingInfo.serving.forEach(updateInfo => {
    ...
    if (existingServing) {
      ...
      existingServing.defaultServingAmount = existingServing.defaultServingAmount || updateInfo.serving_default_serving_amount;
      existingServing.servingAlternateAmount = existingServing.servingAlternateAmount || updateInfo.serving_alternate_amount;
...
async function testMoreFoodCompletion() {
  ...
  const food_result = {
    ...
    "description": "Intense Dark 72% Cacao Dark Chocolate by Ghirardelli offers a rich and luxurious chocolate experience with a high cacao content, providing 170 calories, 2g of protein, 15g of fat, and 14g of carbs per serving.",
    "serving": [
      {
        "serving_id": 0,
