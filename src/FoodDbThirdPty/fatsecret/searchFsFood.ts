...
async function runTest() {
  ...
  const search_term = "Egg Whites"
  const bgeEmbedding = await getCachedOrFetchEmbeddings("BGE_BASE",[search_term])
  const searchParams: FatSecretFindFoodParams = { search_expression: search_term, queryBgeBaseEmbedding: bgeEmbedding[0].embedding }
